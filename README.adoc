...
It will respond with a JSON representation of a units, as the following listing shows:
...
You can customize the units with an optional `name` parameter in the query string, as
the following listing shows:

====
[source,text]
----
http://localhost:8080/units?name=User
----
====
...
The service will handle `GET` requests for `/units`, optionally with a `name` parameter
in the query string. The `GET` request should return a `200 OK` response with JSON in the
body that represents a units. It should resemble the following output:
...
The `id` field is a unique identifier for the units, and `content` is the textual
representation of the units.

To model the units representation, create a resource representation class. To do so,
...
`src/main/java/com/example/restservice/Units.java`) shows:

[source,java]
----
include::complete/src/main/java/com/example/restservice/Units.java[]
----

====
NOTE: This application uses the https://github.com/FasterXML/jackson[Jackson JSON] library to
automatically marshal instances of type `Units` into JSON. Jackson is included by default by the web starter.
====
...
annotation, and the `UnitsController` shown in the following listing (from
`src/main/java/com/example/restservice/UnitsController.java`) handles `GET` requests
for `/units` by returning a new instance of the `Units` class:

====
[source,java]
----
include::complete/src/main/java/com/example/restservice/UnitsController.java[]
----
====
...
The `@GetMapping` annotation ensures that HTTP GET requests to `/units` are mapped to the `units()` method.
...
parameter of the `units()` method. If the `name` parameter is absent in the request,
the `defaultValue` of `World` is used.

The implementation of the method body creates and returns a new `Units` object with
`id` and `content` attributes based on the next value from the `counter` and formats the
given `name` by using the units `template`.
...
relying on a view technology to perform server-side rendering of the units data to
HTML, this RESTful web service controller populates and returns a `Units` object. The
...
The `Units` object must be converted to JSON. Thanks to Spring's HTTP message converter
...
is automatically chosen to convert the `Units` instance to JSON.
...
Now that the service is up, visit `http://localhost:8080/units`, where you should see:
...
`http://localhost:8080/units?name=User`. Notice how the value of the `content`
...
This change demonstrates that the `@RequestParam` arrangement in `UnitsController` is
...
working against the same `UnitsController` instance across multiple requests and that
